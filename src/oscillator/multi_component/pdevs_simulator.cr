module DEVS
  module PDEVS
    module MultiComponent
      class Simulator < DEVS::MultiComponent::Simulator

        @state_bags = Hash(DEVS::MultiComponent::Component,Array(Tuple(Name,Any))).new { |h,k| h[k] = Array(Tuple(Name,Any)).new }
        @imm : Array(DEVS::MultiComponent::Component)?
        ...
        def perform_transitions(time, bag)
          ...
          @state_bags.each do |component, states|
            if @event_set.is_a?(RescheduleEventSet)
              ...
              @event_set.delete(component) if tn < DEVS::INFINITY && time != tn
              component.reaction_transition(states)
              component.time_last = component.time = time - component.elapsed
              tn = component.time_next = component.time_last + component.time_advance
              @event_set.push(component) if tn < DEVS::INFINITY
            end
